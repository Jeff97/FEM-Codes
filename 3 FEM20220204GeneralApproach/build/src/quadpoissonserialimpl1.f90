...
!#endif
      ...
      DOUBLE PRECISION :: xc, yc, fact, val
      ...
      DOUBLE PRECISION, DIMENSION(:), ALLOCATABLE :: soln, solnDot, solnLocal, solnDotLocal
      ...
      DO ii=1,nElem
        READ(1,*, iostat=io) nn, n1, n2, n3, n4
      ...
      !nsize=npElem*ndof
      !DO ee=1,nElem
      !  write(*,*)  ElemDofArray(ee,1), ElemDofArray(ee,2), ElemDofArray(ee,3), ElemDofArray(ee,4)
      !END DO
    ...
    !   ! pcout << " nz_unneeded  = " <<  info.nz_unneeded << endl;
      ...
      ind = nNode*ndof
      ALLOCATE(soln(ind))
      ALLOCATE(solnDot(ind))
      soln = 0.0
      solnDot = 0.0

      ALLOCATE(solnLocal(npElem))
      ALLOCATE(solnDotLocal(npElem))

      write(*,*) " Generating element matrices and vectors "

      ! material data
      elemData(1) = 1;      elemData(2) = 1;
      ...
      DO ee=1,nElem
          ...
          DO ii=1, npElem
            ...
            solnLocal(ii)    = soln(n1)
            solnDotLocal(ii) = solnDot(n1)
          END DO

          ! Compute the element force vector, including residual force
          call StiffnessResidualPoissonLinearQuad(xNode, yNode, elemData, timeData, solnLocal, solnDotLocal, Klocal, Flocal)

          !write(*,*) Flocal
          ! Print the stiffness matrix, if you want
          !DO ii=1,nsize
            !write(*,*) Klocal(ii,1), Klocal(ii,2), Klocal(ii,3), Klocal(ii,4)
          !END DO
          !write(*,*) "\n"
      ...
      call writeoutputvtk(ndim, nElem, nNode, npElem, ndof, coords, elemNodeConn, elem_procid, soln, outFileName)
      ...
      IF( ALLOCATED(solnDot) )       DEALLOCATE(solnDot)
      IF( ALLOCATED(solnLocal) )     DEALLOCATE(solnLocal)
      IF( ALLOCATED(solnDotLocal) )  DEALLOCATE(solnDotLocal)
